...
def initialize_session_state():
    if 'std_dot' not in st.session_state:
        st.session_state['std_dot'] = 1.0  # Non-zero default
    if 'traditional_result' not in st.session_state:
        st.session_state['traditional_result'] = 1.001  # Slightly different to avoid division by zero
    if 'b12_result' not in st.session_state:
        st.session_state['b12_result'] = 1.0
    if 'stats' not in st.session_state:
        st.session_state['stats'] = {'ops_total': 24, 'renorm_local': 2, 'max_abs_r12': 1e-5}
    if 'residue_history' not in st.session_state:
        st.session_state['residue_history'] = [1e-6] * 24  # Sample data
    if 'renorm_events' not in st.session_state:
        st.session_state['renorm_events'] = [11, 23]  # Sample events
    if 'simulation_run' not in st.session_state:
        st.session_state['simulation_run'] = False

# Safe calculation functions
def safe_calculate_energy_savings():
    try:
        if st.session_state.get('stats', {}).get('ops_total', 0) > 0:
            ratio = st.session_state['stats']['renorm_local'] / st.session_state['stats']['ops_total']
            return (1 - ratio) * 30
        return 20.0  # Default fallback
    except:
        return 20.0

def safe_calculate_accuracy_improvement():
    try:
        std_dot = st.session_state.get('std_dot', 1.0)
        traditional_result = st.session_state.get('traditional_result', 1.001)
        b12_result = st.session_state.get('b12_result', 1.0)
        
        traditional_error = abs(std_dot - traditional_result)
        b12_error = abs(std_dot - b12_result)
        
        if traditional_error > 1e-10:  # Avoid division by very small numbers
            improvement = ((traditional_error - b12_error) / traditional_error) * 100
            return max(0, min(100, improvement))  # Clamp between 0-100%
...
if st.session_state.get('simulation_run', False):
    ...
    energy_savings_pct = safe_calculate_energy_savings()
    accuracy_improvement_pct = safe_calculate_accuracy_improvement()
    
    st.markdown(f"""
    **Key Wins**:
    - Operations Handled: {st.session_state['stats']['ops_total']}
    - Fixes Required: {st.session_state['stats']['renorm_local']} (Minimal = Cost Savings)
    - Max Risk Buildup: {st.session_state['stats']['max_abs_r12']:.2e} (Controlled Automatically)
    ...
    if st.session_state.get('residue_history') and len(st.session_state['residue_history']) > 0:
        ...
        renorm_df = pd.DataFrame({'Fix Point': st.session_state['renorm_events']}) if st.session_state.get('renorm_events') else pd.DataFrame({'Fix Point': []})
        ...
        efficiency_score = 100 - (st.session_state['stats']['renorm_local'] / max(st.session_state['stats']['ops_total'], 1) * 100)
        ...
        with col1:
            ...
            st.altair_chart(chart + danger_line, use_container_width=True)
        ...
        with col3:
            ...
            base = alt.Chart(gauge_df).mark_arc(color="lightgray", innerRadius=50, outerRadius=60, theta=6.28, theta2=0)
            gauge = alt.Chart(gauge_df).mark_arc(innerRadius=50, outerRadius=60).encode(
                theta=alt.value(0),
                theta2=alt.Scale(domain=[0,100], range=[0, 6.28]),
                color=alt.Color("Score:Q", scale=alt.Scale(domain=[0,100], range=['red', 'yellow', 'green']))
            ).transform_calculate(
                theta2_calc="datum.Score * 6.28 / 100"
            )
...
st.subheader("Estimate Your ROI with Base-12")
...
if st.session_state.get('simulation_run', False):
    st.markdown(f"""
    **Simulation-Based Values**: Energy Savings ~{energy_savings_pct:.0f}%, Accuracy Improvement ~{accuracy_improvement_pct:.0f}%
    """)
else:
    st.markdown("""
    **Default Estimates**: Run the simulation above for values based on your chosen parameters.
    """)
...
if st.button("Calculate ROI"):
    ...
    payback_period = annual_spend / total_roi if total_roi > 0 else float('inf')
