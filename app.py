...
if st.button("Run Simulation"):
    ...
    energy_savings = max(0, (abs(std_dot - traditional_result) - abs(std_dot - b12_result)) / abs(std_dot - traditional_result) * 100 if abs(std_dot - traditional_result) > 0 else 100)  # Placeholder % improvement
    st.markdown(f"""
    **Key Wins**:
    ...
    - Estimated Improvement: ~{energy_savings:.0f}% Better Accuracy (Translates to Lower Costs)
    """)
...
energy_savings_pct = st.slider("Assumed Energy Savings (%)", min_value=10, max_value=50, value=25, help="Based on our tests; adjust for your scenario.")
accuracy_improvement_pct = st.slider("Assumed Accuracy Improvement (%)", min_value=5, max_value=30, value=15, help="Reduces failed runs/retrains.")

if st.button("Calculate ROI"):
    savings = annual_spend * (energy_savings_pct / 100)
    accuracy_savings = annual_spend * (accuracy_improvement_pct / 100) * 0.2  # Assume 20% of spend is on failures
    ...
    **Net ROI Year 1**: {energy_savings_pct + accuracy_improvement_pct}% return on investment.
